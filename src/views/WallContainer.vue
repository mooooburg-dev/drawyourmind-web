  ...
  <div class="wall-wrapper">
    ...
    <transition name="fade">
      <div v-if="issues.items && issues.items.length > 0" class="issues-container">
        <div class="issues-logo mb-3" style="cursor:pointer;" @click.prevent="issueLogoClick">
        ...
        <template v-for="(item, item_idx) in issues.items">
          <div v-if="!item.title.includes('Bump') && item.state === 'open'" class="issue-item" :key="item_idx">
            <a :href="item.html_url" target="_blank">
              > {{ item.title }}
              <span style="font-size:11px;">[{{ getRepositoryNm(item.repository_url) }}] {{ $moment(item.updated_at).format('YYYY/MM/DD') }}</span>
            </a>
          </div>
        </template>
      </div>
  ...
  data() {
    return {
      fixedIssues: [
        ...
        {
          title: '타임라인 구상중',
          html_url: 'https://www.notion.so/drawyourmind/Timeline-57f4ad0d1d1e40c2b1be068fe0fd09a0',
